\documentclass[
parskip=half,
fontsize=11pt, % fontsize
twoside=off, % onesided layout
version=last,
foldmarks=false,
fromalign=left, % center, if no logo is used, else left
fromrule=true,
frommobilephone,
fromemail,
fromlogo, % if a logo should be used, delete it otherwise
subject=titled,
backaddress=false,
]    
{scrlttr2}

\usepackage[ngerman]{babel}
\usepackage[utf8]{inputenc} % text encoding: UTF-8
\usepackage[T1]{fontenc}
\usepackage[scaled]{helvet}
\renewcommand{\familydefault}{\sfdefault}

\newcommand{\toppagemargin}{2cm}
\newcommand{\rightpagemargin}{2.5cm}
\newcommand{\leftpagemargin}{2cm}
\newcommand{\bottompagemargin}{2cm}
\newcommand{\footerheight}{12mm}

\usepackage[
	a4paper,
	head=14.5pt,
	top=\toppagemargin,
	bottom=\bottompagemargin,
	inner=\leftpagemargin,
	outer=\rightpagemargin
]{geometry}
\usepackage{graphicx}

\usepackage{hyperref}

% custom commands
\newcommand{\monthword}[1]{\ifcase#1\or Januar\or Februar\or März\or April\or
	Mai\or Juni\or Juli\or August\or
	September\or Oktober\or November\or Dezember\fi} 
\newcommand{\leadingzero}[1]{\ifnum #1<10 0\the#1\else\the#1\fi}
\newcommand{\todaay}{\leadingzero{\day}.~\monthword{\the\month} \the\year}

% --------------------------------------------------------------------------------
% set koma variables, this needs to be edited by the user
\newcommand{\mail}{john.smith@example.com}
\setkomavar{fromname}[]{John Smith}
\setkomavar{fromzipcode}{12345}
\setkomavar{place}{Ort}
\setkomavar{signature}{\includegraphics[scale=0.05]{src/fig/signature.png}\\\usekomavar{fromname}}
\setkomavar{fromaddress}{Straße 1\\
\usekomavar{fromzipcode} \usekomavar{place}}
\setkomavar{frommobilephone}{+1~234~56789}
\setkomavar{subject}{Bewerbung~als~<POSITION>}
% --------------------------------------------------------------------------------
\setkomavar{firstfoot}{%
\parbox[b]{\linewidth}{%
\centering\def\\{, }\usekomavar{fromname}, \usekomavar{fromemail}%
}%
}
\setkomavar{fromlogo}{\includegraphics{src/fig/logo.pdf}} % delete this if no logo should be used
\setkomavar{fromemail}{\href{mailto:\mail}{\mail}}
\setkomavar{date}{\todaay}

% Layout
\setplength{refvpos}{5.5cm}
\setplength{subjectaftervskip}{\baselineskip}
\setplength{toaddrvpos}{2cm}
\setplength{sigbeforevskip}{0.2cm}

% --------------------------------------------------------------------------------
% edit the companies address
\newcommand{\address}{
Firmenname    \\
Firmenadresse \\
Kontakt       \\
}
% --------------------------------------------------------------------------------

\begin{document}

\begin{letter}{\address}
	\opening{Sehr~geehrte~Damen~und~Herren}

	Hier den Text für das Anschreiben einfügen, max. 1 Seite.
	\closing{Freundliche Grüße}

	\encl{
		Anlagen, die in der Bewerbung dabei sind
	}

\end{letter}

\end{document}
